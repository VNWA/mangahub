<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h2 class="text-2xl font-bold text-zinc-900 dark:text-white">Quản lý Coin</h2>
      <div
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-md p-6">
      <h3 class="text-lg font-semibold text-zinc-900 dark:text-white mb-4">Nạp Coin</h3>
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">
            Số lượng coin
          </label>
          <UInput v-model.number="depositAmount" type="number" min="1" placeholder="Nhập số coin muốn nạp"
            :disabled="depositing" />
        </div>
        <div>
          <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">
          ...
          <UInput v-model="depositDescription" placeholder="Ví dụ: Nạp coin để mở khóa chapter"
            :disabled="depositing" />
        </div>
        <UButton @click="handleDeposit" :loading="depositing" :disabled="!depositAmount || depositAmount < 1"
    ...
    <div class="border-b border-zinc-200 dark:border-zinc-700">
      <div class="flex gap-6">
        ...
        <button @click="activeTab = 'unlocks'" :class="[
          'py-3 px-2 font-semibold text-sm border-b-2 transition-colors',
          activeTab === 'unlocks'
            ? 'border-primary text-primary'
            : 'border-transparent text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white'
    ...
    <!-- Transactions Tab -->
    <div v-if="activeTab === 'transactions'" class="bg-white dark:bg-zinc-800 rounded-lg shadow-md p-6">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-semibold text-zinc-900 dark:text-white">Lịch sử giao dịch</h3>
        <USelect v-model="transactionType" :options="transactionTypeOptions" value-attribute="value"
          option-attribute="label" size="sm" class="w-40" @change="loadTransactions" />
      </div>
      ...
      <div v-else-if="transactions.length === 0" class="text-center py-8">
        <UIcon name="i-heroicons-document-text" class="w-12 h-12 text-zinc-400 mx-auto mb-3" />
        <p class="text-zinc-600 dark:text-zinc-400">Chưa có giao dịch nào</p>
      </div>
    ...
    <!-- Unlocks Tab -->
    <div v-if="activeTab === 'unlocks'" class="bg-white dark:bg-zinc-800 rounded-lg shadow-md p-6">
      <h3 class="text-lg font-semibold text-zinc-900 dark:text-white mb-4">Lịch sử mở khóa</h3>
      ...
      <div v-else-if="unlocks.length === 0" class="text-center py-8">
        <UIcon name="i-heroicons-lock-open" class="w-12 h-12 text-zinc-400 mx-auto mb-3" />
        <p class="text-zinc-600 dark:text-zinc-400">Chưa mở khóa chapter nào</p>
      </div>

      <div v-else class="space-y-3">
        <div v-for="unlock in unlocks" :key="unlock.id"
          class="flex items-center gap-4 p-4 border border-zinc-200 dark:border-zinc-700 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-700/50 transition-colors">
          <div class="w-12 h-16 bg-zinc-200 dark:bg-zinc-700 rounded flex-shrink-0"></div>
          <div class="flex-1 min-w-0">
            <NuxtLink :to="`/${unlock.manga.slug}/${unlock.chapter.slug}`"
              class="font-medium text-zinc-900 dark:text-white hover:text-primary block truncate">
              {{ unlock.chapter.name }}
            </NuxtLink>
            <NuxtLink :to="`/${unlock.manga.slug}`"
              class="text-sm text-zinc-600 dark:text-zinc-400 hover:text-primary block truncate">
              {{ unlock.manga.name }}
            </NuxtLink>
          ...
          <div class="text-right flex-shrink-0">
            ...
            <p class="text-xs text-zinc-500 dark:text-zinc-400">coin</p>
          </div>
        </div>
      </div>

        ...
        <main class="min-h-screen bg-white dark:bg-slate-900 py-8">
            <div class="container mx-auto px-4 md:px-6 lg:px-8 max-w-7xl">
                ...
                <nav class="mb-8 flex items-center gap-2 text-sm">
                    ...
                    <span class="text-slate-900 dark:text-white">{{ story?.title || 'Loading...' }}</span>
                </nav>
                ...
                <!-- Grid Layout -->
                <div v-else-if="story" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <!-- Sidebar -->
                    <div class="lg:col-span-1">
                        <div class="sticky top-4 space-y-6">
                            <!-- Story Info -->
                            <div class="bg-white dark:bg-slate-800 rounded-lg shadow-md overflow-hidden">
                                <img :src="story.coverImage || 'https://via.placeholder.com/400x600'" :alt="story.title"
                                    class="w-full h-64 object-cover" />
                                <div class="p-4">
                                    <h2 class="font-bold text-slate-900 dark:text-white mb-1">{{ story.title }}</h2>
                                    <p class="text-sm text-slate-600 dark:text-slate-400 mb-4">{{ story.author }}</p>
                                    ...
                                    <div class="grid grid-cols-2 gap-3 mb-4 text-center text-sm">
                                        ...
                                        <div>
                                            <div class="font-bold">{{ formatNumber(story.views) }}</div>
                            ...
                            <!-- Quick Stats -->
                            <div class="bg-white dark:bg-slate-800 rounded-lg shadow-md p-4 space-y-3">
                                ...
                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-slate-600 dark:text-slate-400">Cập nhật</span>
                                    <span class="text-sm font-semibold">{{ story.lastUpdate }}</span>
                                </div>
                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-slate-600 dark:text-slate-400">Chương mới</span>
                                    <span class="text-sm font-semibold">{{ story.lastChapter }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-2 space-y-8">
                        ...
                        <div>
                            ...
                            <p class="text-lg text-slate-600 dark:text-slate-400 mb-6">{{ story.author }}</p>

                            <div class="mb-6">
                                <div class="flex flex-wrap gap-2 mb-4">
                                    <UBadge v-for="cat in story.categories" :key="cat" :label="cat" variant="soft"
                                        size="sm" />
                                </div>
                            </div>
                        ...
                        <!-- Related Stories -->
                        <div v-if="story && relatedStories.length > 0"
                            class="border-t border-slate-200 dark:border-slate-700 pt-8">
                            <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-6">Truyện liên quan</h3>
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <StoryCard v-for="s in relatedStories" :key="s.id" :story="s" variant="grid" />
                            </div>
                        </div>
                    </div>
                </div>
...
const readFromStart = () => {
    if (chapters.value.length > 0 && chapters.value[0]) {
        navigateTo(`/${story.value?.slug}/${chapters.value[0].slug}`)
    }
}

useHead({
    title: computed(() => `${story.value?.title || 'Loading...'} - Đọc Truyện Online`),
    meta: [
        {
            name: 'description',
            content: computed(() => story.value?.description?.substring(0, 150) || '')
        }
    ]
})
</script>
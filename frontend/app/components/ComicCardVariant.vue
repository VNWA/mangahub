<template>
  ...
  <article
    v-if="variant === 'list'"
    class="bg-white dark:bg-slate-800 rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-all p-4 flex gap-4"
  >
    <div class="flex-shrink-0 w-24 h-32">
      <Image
        :src="comic.thumbnail"
        :alt="`${comic.title} thumbnail`"
        container-class="w-24 h-32"
        hoverable
      />
    </div>
    <div class="flex-1 flex flex-col justify-between">
      <div>
        <div class="flex items-start justify-between mb-2">
          <h3 class="text-lg font-bold text-slate-900 dark:text-white line-clamp-2 flex-1">
            {{ comic.title }}
          </h3>
          <UBadge
            :label="comic.status"
            :color="comic.status === 'Đang cập nhật' ? 'success' : 'neutral'"
            variant="subtle"
            size="sm"
          />
        </div>
        <p class="text-sm text-slate-600 dark:text-slate-400 mb-2">{{ comic.author }}</p>
        <p class="text-sm text-slate-700 dark:text-slate-300 line-clamp-2">{{ comic.description }}</p>
      </div>
      <div class="flex items-center justify-between pt-2">
        <div class="flex gap-3">
          ...
          <div class="text-sm text-slate-600 dark:text-slate-400">{{ comic.views }} lượt xem</div>
        </div>
        <UButton
          to="`/comic/${comic.id}`"
          color="primary"
  ...
  >
    ...
    <div class="p-4">
      ...
      <p class="text-sm text-slate-600 dark:text-slate-400 line-clamp-2 mb-3">{{ comic.author }}</p>
      ...
      <p class="text-xs text-slate-500 dark:text-slate-400 mb-3">{{ comic.lastChapter }}</p>
      <UButton
        to="`/comic/${comic.id}`"
        color="primary"
        variant="soft"
  ...
  <article
    ...
    class="bg-white dark:bg-slate-800 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all group"
  >
    ...
    <div class="p-3">
      <h4 class="font-semibold text-slate-900 dark:text-white line-clamp-1 text-sm mb-1">{{ comic.title }}</h4>
  ...
  <article
    ...
    class="relative rounded-lg overflow-hidden group h-96 bg-slate-200 dark:bg-slate-700"
  >
    <Image
      :src="comic.thumbnail"
      :alt="`${comic.title} thumbnail`"

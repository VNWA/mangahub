<template>
  ...
  >
    ...
    <NuxtLink :to="`/${comic.slug || comic.id}`" class="relative overflow-hidden bg-gray-200 dark:bg-slate-700 h-64 block">
      <img
    ...
    <div class="p-4 flex-1 flex flex-col">
      <NuxtLink :to="`/${comic.slug || comic.id}`" class="flex-1">
        <h3 class="text-lg font-bold text-slate-900 dark:text-white line-clamp-2 mb-2 hover:text-purple-600 dark:hover:text-purple-400 transition-colors">
          {{ comic.title }}
        </h3>
      ...
      <NuxtLink
        v-if="comic.lastChapter"
        :to="`/${comic.slug || comic.id}/${getLastChapterNumber(comic.lastChapter)}`"

<script setup lang="ts">
import AppLayout from '@/layouts/AppLayout.vue';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { Separator } from '@/components/ui/separator';
import { Head, Link, router } from '@inertiajs/vue3';
import { useToast } from 'vue-toastification';
import { ref } from 'vue';
import axios from '@/axios';
import {
    Award,
    Plus,
    Search,
    X,
    Edit,
    Trash2,
    Eye,
    MoreVertical,
    BookOpen,
} from 'lucide-vue-next';
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
    DropdownMenuTrigger,
} from '@/components/ui/dropdown-menu';
import badges from '@/routes/badges';

const toast = useToast();

interface BadgeItem {
    id: number;
    name: string;
    slug: string;
    light_text_color: string;
    light_bg_color: string;
    dark_text_color: string;
    dark_bg_color: string;
    mangas_count?: number;
    is_system_badge?: boolean;
}

interface Props {
    badges: {
        data: BadgeItem[];
        current_page: number;
        last_page: number;
        per_page: number;
        total: number;
        links: Array<{
            url: string | null;
            label: string;
            active: boolean;
        }>;
    };
    filters: {
        search?: string;
    };
}

const props = defineProps<Props>();

const searchQuery = ref(props.filters.search || '');

const breadcrumbs = [
    { title: 'Dashboard', href: '/admin/dashboard' },
    { title: 'Badges', href: '#' },
];

const handleSearch = () => {
    router.get(
        badges.index().url,
        {
            search: searchQuery.value || undefined,
        },
        {
            preserveState: true,
            replace: true,
        },
    );
};

const handleDelete = async (badgeId: number) => {
    if (!confirm('Bạn có chắc chắn muốn xóa badge này?')) {
        return;
    }

    try {
        const response = await axios.delete(badges.destroy(badgeId).url);
        if (response.data?.success) {
            toast.success(response.data.message || 'Badge đã được xóa thành công.');
            router.reload({ only: ['badges'] });
        }
    } catch (error: any) {
        toast.error(error.response?.data?.message || 'Có lỗi xảy ra khi xóa badge.');
    }
};

const clearFilters = () => {
    searchQuery.value = '';
    router.get(badges.index().url, {}, { preserveState: true, replace: true });
};
</script>

<template>
    <Head title="Quản lý Badges" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-6">
            <!-- Header -->
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="space-y-1">
                    <div class="flex items-center gap-3">
                        <h1 class="text-3xl font-bold tracking-tight">Quản lý Badges</h1>
                        <Badge variant="secondary" class="text-sm">
                            {{ props.badges.total }} badges
                        </Badge>
                    </div>
                    <p class="text-muted-foreground">Quản lý tất cả các badge cho manga</p>
                </div>
                <Button as-child>
                    <Link :href="badges.create().url">
                        <Plus class="mr-2 h-4 w-4" />
                        Thêm Badge
                    </Link>
                </Button>
            </div>

            <!-- Filters -->
            <Card>
                <CardContent class="p-4">
                    <div class="flex gap-2">
                        <div class="relative flex-1">
                            <Search class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                            <Input
                                v-model="searchQuery"
                                type="text"
                                placeholder="Tìm kiếm badge..."
                                class="pl-9 pr-9"
                                @keyup.enter="handleSearch"
                            />
                            <button
                                v-if="searchQuery"
                                @click="searchQuery = ''"
                                class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground"
                            >
                                <X class="h-4 w-4" />
                            </button>
                        </div>
                        <Button variant="outline" @click="handleSearch">
                            <Search class="mr-2 h-4 w-4" />
                            Tìm
                        </Button>
                        <Button
                            v-if="searchQuery"
                            variant="outline"
                            @click="clearFilters"
                        >
                            <X class="mr-2 h-4 w-4" />
                            Xóa
                        </Button>
                    </div>
                </CardContent>
            </Card>

            <Separator />

            <!-- Badges List -->
            <div v-if="props.badges.data.length > 0" class="space-y-4">
                <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    <Card
                        v-for="badge in props.badges.data"
                        :key="badge.id"
                        class="group relative transition-all hover:shadow-lg"
                    >
                        <CardContent class="p-6">
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    <div class="mb-3">
                                        <Badge
                                            :style="{
                                                color: badge.light_text_color,
                                                backgroundColor: badge.light_bg_color,
                                            }"
                                            class="text-sm font-semibold"
                                        >
                                            {{ badge.name }}
                                        </Badge>
                                    </div>
                                    <div class="flex items-center gap-2 mb-2">
                                        <h3 class="text-lg font-semibold">{{ badge.name }}</h3>
                                        <Badge v-if="badge.is_system_badge" variant="secondary" class="text-xs">
                                            Hệ thống
                                        </Badge>
                                    </div>
                                    <div class="space-y-2 text-xs text-muted-foreground">
                                        <div class="flex items-center gap-2">
                                            <span>Light:</span>
                                            <div class="flex items-center gap-1">
                                                <div
                                                    class="h-4 w-4 rounded border"
                                                    :style="{ backgroundColor: badge.light_bg_color }"
                                                />
                                                <div
                                                    class="h-4 w-4 rounded border"
                                                    :style="{ backgroundColor: badge.light_text_color }"
                                                />
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <span>Dark:</span>
                                            <div class="flex items-center gap-1">
                                                <div
                                                    class="h-4 w-4 rounded border"
                                                    :style="{ backgroundColor: badge.dark_bg_color }"
                                                />
                                                <div
                                                    class="h-4 w-4 rounded border"
                                                    :style="{ backgroundColor: badge.dark_text_color }"
                                                />
                                            </div>
                                        </div>
                                    </div>
                                    <Badge variant="outline" class="mt-3 text-xs">
                                        <BookOpen class="mr-1 h-3 w-3" />
                                        {{ badge.mangas_count || 0 }} manga
                                    </Badge>
                                </div>
                                <DropdownMenu>
                                    <DropdownMenuTrigger as-child>
                                        <Button variant="ghost" size="sm" class="h-8 w-8 p-0 shrink-0">
                                            <MoreVertical class="h-4 w-4" />
                                        </Button>
                                    </DropdownMenuTrigger>
                                    <DropdownMenuContent align="end">
                                        <DropdownMenuItem as-child>
                                            <Link :href="badges.show(badge.id).url">
                                                <Eye class="mr-2 h-4 w-4" />
                                                Xem chi tiết
                                            </Link>
                                        </DropdownMenuItem>
                                        <DropdownMenuItem as-child>
                                            <Link :href="badges.edit(badge.id).url">
                                                <Edit class="mr-2 h-4 w-4" />
                                                Chỉnh sửa
                                            </Link>
                                        </DropdownMenuItem>
                                        <DropdownMenuItem
                                            v-if="!badge.is_system_badge"
                                            class="text-destructive"
                                            @click="handleDelete(badge.id)"
                                        >
                                            <Trash2 class="mr-2 h-4 w-4" />
                                            Xóa
                                        </DropdownMenuItem>
                                        <DropdownMenuItem
                                            v-else
                                            disabled
                                            class="text-muted-foreground"
                                        >
                                            <Trash2 class="mr-2 h-4 w-4" />
                                            Không thể xóa badge hệ thống
                                        </DropdownMenuItem>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                            </div>
                        </CardContent>
                    </Card>
                </div>

                <!-- Pagination -->
                <div v-if="props.badges.last_page > 1" class="flex items-center justify-center gap-2">
                    <template v-for="(link, index) in props.badges.links" :key="index">
                        <Button
                            v-if="link.url"
                            variant="outline"
                            size="sm"
                            :disabled="link.active"
                            @click="router.get(link.url!)"
                            :class="{ 'bg-primary text-primary-foreground': link.active }"
                        >
                            <span v-html="link.label" />
                        </Button>
                        <span v-else class="px-2 text-muted-foreground">
                            <span v-html="link.label" />
                        </span>
                    </template>
                </div>
            </div>

            <!-- Empty State -->
            <Card v-else>
                <CardContent class="flex flex-col items-center justify-center py-16">
                    <div class="rounded-full bg-muted p-6">
                        <Award class="h-12 w-12 text-muted-foreground" />
                    </div>
                    <div class="mt-4 space-y-2 text-center">
                        <h3 class="text-lg font-semibold">Chưa có badge nào</h3>
                        <p class="text-sm text-muted-foreground">
                            Bắt đầu bằng cách thêm badge đầu tiên
                        </p>
                    </div>
                    <Button class="mt-4" as-child>
                        <Link :href="badges.create().url">
                            <Plus class="mr-2 h-4 w-4" />
                            Thêm Badge
                        </Link>
                    </Button>
                </CardContent>
            </Card>
        </div>
    </AppLayout>
</template>
